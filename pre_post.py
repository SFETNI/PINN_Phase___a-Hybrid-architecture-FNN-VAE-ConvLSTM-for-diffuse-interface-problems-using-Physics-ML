...
class PrePost:
   ...
   ###############################################
    ...
    ###################################
    def get_phi_values(self,R0,eta,r,i_cir):
        phi_inf = 0
        ...
        return phi_values   
    ###############################################
    def get_X_ini_values(self,R0,eta,r,i_cir):
        phi_inf = 0
        phi_mid = 0.5 - 0.5*np.sin(np.pi*(r-R0[i_cir])/eta)
        phi_sup = 1.0
    ...
    ###############################################
    def init_micro_cir(self,R0,eta,ox,oy,oz,Nx,Ny,x,y):
        ...
        for i_cir in range(len(R0)): 
            ...
            for i_coory in range(Ny):
                for i_coorx in range(Nx):
                        ...
                        phi=self.get_phi_values(R0,eta,r,i_cir)
                        if phi>0:
    ...
    def set_training_data(self,x,y,N_ini,all_phases, all_interfaces,\
        ...
        for idx in range(self.num_phases):
            ...
            plt.scatter(X_mat[:, 0], X_mat[:, 1], cmap=plt.get_cmap('viridis'), c=phi_mat[:,idx])
            plt.title(f"Phi of Phase {idx}")
